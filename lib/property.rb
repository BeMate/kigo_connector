module KigoConnector
  ...
  class Property
    ...
    # checkin must be greater than today.
    # checkout must be grater than checkin.
    def real_time_pricing_calculaton(checkin, checkout, guests)
      if Date.parse(checkin) > Date.today && Date.parse(checkout) > Date.parse(checkin)
        ...
        raise Exception.new("Date constrains not met.")
      end
    end
